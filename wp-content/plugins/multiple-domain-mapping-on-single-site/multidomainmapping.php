    ...
    public static function get_instance(){
	  ...
	  public function set_textdomain(){
		...
		public function replace_domain($input){
			...
			if(!empty($this->getCurrentMapping()['match'])){
				//but we need to make sure that we only replace right at the beginning (after the protocol), so we do not destroy subdomains (like img.mydomain.com)
				//that is why we add the :// to the strings
				$input = str_ireplace( '://' . parse_url(get_site_url())['host'], '://' . parse_url('dummyprotocol://' . $this->getCurrentMapping()['match']['domain'])['host'], $input);
			}
			return $input;
		}
